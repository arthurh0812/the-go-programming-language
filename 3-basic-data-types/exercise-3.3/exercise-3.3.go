// surface4 computes an SVG rendering of a 3-D surface function.
package main
...
func renderSVG(w http.ResponseWriter, req *http.Request) {
	...
	for i := 0; i < cells; i++ {
		for j := 0; j < cells; j++ {
			...
			avgHeight := (az + bz + cz + dz) / 4

			if avgHeight > maxHeight {
				maxHeight = avgHeight
			} else if avgHeight < minHeight {
				minHeight = avgHeight
			}
...
func color(avgHeight float64) {
	// rnge := math.Abs(maxHeight - minHeight)
